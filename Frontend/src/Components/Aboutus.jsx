...
import { FaLinkedinIn, FaTwitter, FaGithub, FaHeart, FaInstagram } from "react-icons/fa";
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats({
        ...
        activeToday: Math.floor(Math.random() * 50) + 100 // This would come from backend
      });
    } catch (error) {
    ...
    <div className="max-w-6xl mx-auto py-16 px-6 animate-fade-in">
      ...
      <div className="bg-gradient-to-r from-purple-900/30 via-pink-900/30 to-indigo-900/30 rounded-3xl p-12 text-center border border-gray-700">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link to="/signup">
            <button className="px-8 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition transform hover:scale-105 shadow-lg">
              Get Started Free
            </button>
          </Link>
          <Link to="/journal">
            <button className="px-8 py-3 bg-gray-800 text-gray-200 rounded-lg hover:bg-gray-700 transition transform hover:scale-105 shadow-lg border border-gray-700">
              Try Demo Journal
            </button>
          </Link>
        </div>
